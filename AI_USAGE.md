...
5. **Reconciliation of `kv_cache_util` (0.93):** Reconciled the theoretical decimal VRAM memory budget ($12.08\text{ GB}$) with the engine's allocated block pool, reconciling the modeled KV utilization with the logged 0.93 value at batch 24 (the small difference being attributable to rounding/representation of the logged metric).
