...
1. **Tokenizer Mismatch Invalidates Script Invariance Claims:** `REPORT_v0.md` tested only the English-centric `gpt2` tokenizer (50k vocabulary) and claimed high fertility is an inherent property of the Devanagari script. On our 997-sentence parallel benchmark, Hindi token expansion drops from **7.45× on GPT-2** to **2.53× on LLaMA-3 (128k)** and **1.26× on XLM-RoBERTa (250k)**, disproving the claim that Hindi is inherently 6× more expensive for any tokenizer.
...
4. **Physical Memory Ceiling at Batch 24:** For FLM-4B at 4096 context on 1× L4 (24 GB), maximum concurrent sequence capacity is strictly bounded at **25 to 28 streams**. At batch 48, severe memory saturation triggers **23 sequence preemptions**, cutting throughput to $1298.5\text{ tok/s}$ and inflating p95 latency to $105.4\text{ seconds}$.
5. **Prompt Engineering Is the Optimal Day-1 Strategy:** Under the constraints of 1 A100 GPU for 2 weeks, a single native reviewer (30 hours total covering Hindi/Kannada only), and a 3-week launch window, Prompt Engineering provides immediate empirical feedback on Day 1 while preserving all 336 GPU-hours if an SFT pivot is required at Day 7.
